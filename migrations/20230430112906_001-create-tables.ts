...
export async function up(knex: Knex): Promise<void> {
  if (!(await knex.schema.hasTable('user'))) {
    await knex.schema.createTable('user', table => {
      ...
      table.string('fps_id', 255).notNullable().unique()
      table.string('payme_link', 255).notNullable().unique()
  ...
  if (!(await knex.schema.hasTable('group_member'))) {
    await knex.schema.createTable('group_member', table => {
      table.increments('id')
      table.integer('member_userid').unsigned().notNullable().unique().references('user.id')
